...
// ===================== SHADER BACKGROUND =====================
const ShaderBackground = ({ colorPhase = [0, 2, 4] }) => {
    const shaderProps = {
        flowSpeed: 0.4,
        colorIntensity: 1.2,
        noiseLayers: 4.0,
...
const InteractiveShader = ({
  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      float map(vec3 p) {
          vec3 q = p;
          // Horizontal flow instead of vertical
          q.x += iTime * uFlowSpeed;
          vec2 mouse = (iMouse.xy / iResolution.xy - 0.5) * 2.0;
          q.xy += mouse * uMouseInfluence;
          float f = fbm(q * 2.0);
          f *= smoothstep(-1.0, 1.0, sin(p.x * 1.0 + iTime));
      ...
      void main() {
        ...
        for (int i=0; i<MARCH_STEPS; i++) {
            ...
            if (density > 0.0) {
                ...
                float firePhase = sin(p.x * 2.0 + iTime) * 0.5 + 0.5;
                vec3 red = vec3(1.0, 0.1, 0.0);
                vec3 orange = vec3(1.0, 0.4, 0.0);
                vec3 yellow = vec3(1.0, 0.8, 0.1);
                vec3 fireColor = mix(red, orange, firePhase);
                fireColor = mix(fireColor, yellow, firePhase * 0.5);

                col += fireColor * density * 0.1 * uColorIntensity;
            }
            t += 0.1;
        }
    ...
    const uColorPhaseLocation = gl.getUniformLocation(program, "uColorPhase");
    ...
    const renderLoop = () => {
      ...
      gl.uniform3f(uColorPhaseLocation, colorPhase[0], colorPhase[1], colorPhase[2]);
  ...
  }, [flowSpeed, colorIntensity, noiseLayers, mouseInfluence, colorPhase]);

  return <canvas ref={canvasRef} className="absolute top-0 left-0 w-full h-full" />;
};

export default AuroraHero;
export { InteractiveShader, ShaderBackground };
